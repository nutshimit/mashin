use self::renderer::{ProgressBarRenderer, ProgressData, ProgressDataDisplayEntry};
use super::draw_thread::{ConsoleSize, DrawThread, DrawThreadGuard, DrawThreadRenderer};
...
impl ProgressMessagePrompt {
	pub fn as_text(&self) -> String {
		match self {
			...
			ProgressMessagePrompt::Initialize => colors::green("Initialize").to_string(),
		}
	}
}
...
impl ProgressBarInner {
	...
	pub fn add_entry(&self, kind: ProgressMessagePrompt, message: String) -> ProgressBarEntry {
	...
	fn finish_entry(&self, entry_id: usize) {
		...
		if let Ok(index) = internal_state.entries.binary_search_by(|e| e.id.cmp(&entry_id)) {
	...
	fn maybe_start_draw_thread(&self, internal_state: &mut InternalState) {
		if internal_state.draw_thread_guard.is_none() && internal_state.keep_alive_count > 0 {
			internal_state.start_time = SystemTime::now();
			internal_state.draw_thread_guard = Some(DrawThread::add_entry(Arc::new(self.clone())));
		}
	}
}
...
impl ProgressBar {
	...
	pub fn new(style: ProgressBarStyle) -> Self {
		Self {
			inner: ProgressBarInner::new(match style {
				ProgressBarStyle::DownloadBars => Arc::new(renderer::BarProgressBarRenderer),
				ProgressBarStyle::TextOnly => Arc::new(renderer::TextOnlyProgressBarRenderer),
			}),
		}
	}
	...
	pub fn update_with_prompt(&self, kind: ProgressMessagePrompt, msg: &str) -> UpdateGuard {
		...
		if ProgressBar::are_supported() {
			...
			UpdateGuard { maybe_entry: Some(entry) }
		} else {
