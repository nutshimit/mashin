	...
	fn render(&self, data: ProgressData) -> String {
		let (bytes_text, bytes_text_max_width) = {
			...
			if total_size == 0 {
				...
				(
					format!(" {}/{}", human_download_size(pos, total_size), total_size_str,),
		...
		let (total_text, total_text_max_width) = if data.total_entries <= 1 {
			...
			(
				format!(" ({}/{})", data.total_entries - data.pending_entries, data.total_entries),
		...
		let total_bars =
			if same_line_text_width > max_width { 1 } else { max_width - same_line_text_width };
		let completed_bars = (total_bars as f64 * data.percent_done).floor() as usize;
		text.push_str(" [");
