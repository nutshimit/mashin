...
mod provider {
	...
	impl ProviderBuilder for Provider {
		async fn build(&mut self) -> mashin_sdk::Result<()> {
			...
			let default_state = State { aws_client: Some(Arc::new(client)) };
	...
	impl mashin_sdk::Resource for Bucket {
		async fn get(&mut self, provider_state: &mashin_sdk::ProviderState) -> Result<()> {
			...
			let bucket_exist = client.head_bucket().bucket(self.name()).send().await.is_ok();
		...
		async fn create(&mut self, _provider_state: &mashin_sdk::ProviderState) -> Result<()> {
			todo!()
		}
		async fn delete(&mut self, _provider_state: &mashin_sdk::ProviderState) -> Result<()> {
			todo!()
		}
		async fn update(
			&mut self,
			_provider_state: &mashin_sdk::ProviderState,
			_diff: &mashin_sdk::ResourceDiff,
		) -> Result<()> {
			todo!()
		}
	}
