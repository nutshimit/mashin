...
class UnsafePointerView {
  ...
  getBool(offset = 0) {
    return ops.op_ffi_read_bool(this.pointer, offset);
  }

  getUint8(offset = 0) {
    return ops.op_ffi_read_u8(this.pointer, offset);
  }

  getInt8(offset = 0) {
    return ops.op_ffi_read_i8(this.pointer, offset);
  }

  getUint16(offset = 0) {
    return ops.op_ffi_read_u16(this.pointer, offset);
  }

  getInt16(offset = 0) {
    return ops.op_ffi_read_i16(this.pointer, offset);
  }

  getUint32(offset = 0) {
    return ops.op_ffi_read_u32(this.pointer, offset);
  }

  getInt32(offset = 0) {
    return ops.op_ffi_read_i32(this.pointer, offset);
  }

  getBigUint64(offset = 0) {
    ops.op_ffi_read_u64(this.pointer, offset, U32_BUFFER);
    return U64_BUFFER[0];
  }

  getBigInt64(offset = 0) {
    ops.op_ffi_read_i64(this.pointer, offset, U32_BUFFER);
    return I64_BUFFER[0];
  }

  getFloat32(offset = 0) {
    return ops.op_ffi_read_f32(this.pointer, offset);
  }

  getFloat64(offset = 0) {
    return ops.op_ffi_read_f64(this.pointer, offset);
  }

  getPointer(offset = 0) {
    return ops.op_ffi_read_ptr(this.pointer, offset);
  }

  getCString(offset = 0) {
    return ops.op_ffi_cstr_read(this.pointer, offset);
  }

  static getCString(pointer, offset = 0) {
    return ops.op_ffi_cstr_read(pointer, offset);
  }

  getArrayBuffer(byteLength, offset = 0) {
    return ops.op_ffi_get_buf(this.pointer, offset, byteLength);
  }

  static getArrayBuffer(pointer, byteLength, offset = 0) {
    return ops.op_ffi_get_buf(pointer, offset, byteLength);
  }
...
class UnsafeFnPointer {
  ...
  call(...parameters) {
    if (this.definition.nonblocking) {
      ...
      if (this.#structSize === null) {
        return ops.op_ffi_call_ptr(this.pointer, this.definition, parameters);
      } else {
        const buffer = new Uint8Array(this.#structSize);
        ops.op_ffi_call_ptr(this.pointer, this.definition, parameters, buffer);
        return buffer;
      }
    }
  }
...
function isStruct(type) {
  return (
    typeof type === "object" && type !== null && typeof type.struct === "object"
  );
}
  ...
  constructor(path, symbols) {
    ...
    for (const symbol in symbols) {
      ...
      if (ReflectHas(symbols[symbol], "type")) {
        ...
        const value = ops.op_ffi_get_static(this.#rid, name, type);
        ObjectDefineProperty(this.symbols, symbol, {
          configurable: false,
          enumerable: true,
          value,
          writable: false,
        ...
        ObjectDefineProperty(this.symbols, symbol, {
          ...
          value: (...parameters) => {
            if (isStructResult) {
              ...
              return PromisePrototypeThen(ret, () => buffer);
            } else {
              return core.opAsync(
                "op_ffi_call_nonblocking",
                this.#rid,
