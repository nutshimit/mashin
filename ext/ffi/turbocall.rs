...
impl From<&NativeType> for fast_api::Type {
    fn from(native_type: &NativeType) -> Self {
        match native_type {
            ...
            NativeType::U8 | NativeType::U16 | NativeType::U32 => fast_api::Type::Uint32,
            NativeType::I8 | NativeType::I16 | NativeType::I32 => fast_api::Type::Int32,
            ...
            NativeType::Struct(_) => fast_api::Type::TypedArray(fast_api::CType::Uint8),
        }
    }
}
...
impl SysVAmd64 {
    ...
    fn compile(sym: &Symbol) -> Trampoline {
        ...
        let must_cast_return_value = compiler.must_cast_return_value(&sym.result_type);
    ...
    fn move_float(&mut self, param: Floating) {
        ...
        let stack_has_moved =
            self.allocated_stack > 0 || self.integral_params >= Self::INTEGRAL_REGISTERS;
        ...
        if is_in_stack && stack_has_moved {
            ...
            debug_assert!(self.allocated_stack == 0 || self.offset_callee <= self.allocated_stack);
        }
        self.float_params += 1;
    }
...
impl Aarch64Apple {
    ...
    fn move_float(&mut self, param: Floating) {
        ...
        if is_in_stack {
            ...
            let padding_callee = (param.size() - self.offset_callee % param.size()) % param.size();
    ...
    fn move_integral(&mut self, param: Integral) {
        ...
        match (param_i, param) {
            ...
            (8.., param) => {
                ...
                let padding_trampl = padding_to_align(size_trampl, self.offset_trampoline);
                let padding_callee = padding_to_align(size_original, self.offset_callee);
                let ot = self.offset_trampoline;
                let oc = self.offset_callee;
                match param {
                    I(B) | U(B) => aarch64!(s
...
impl From<NativeType> for Param {
    fn from(native: NativeType) -> Self {
        match native {
            ...
            NativeType::U64 | NativeType::USize | NativeType::Pointer | NativeType::Function => {
                Int(U(QW))
            }
...
mod tests {
    ...
    mod sysv_amd64 {
        ...
        fn tailcall() {
            let trampoline = SysVAmd64::compile(&symbol(
                vec![
                    U8, U16, I16, I8, U32, U64, Buffer, Function, I64, I32, I16, I8, F32, F32, F32,
        ...
        fn return_u64_in_stack_typed_array() {
            let trampoline =
                SysVAmd64::compile(&symbol(vec![U64, U64, U64, U64, U64, U64, U64], U64));
    ...
    mod x64_windows {
        ...
        #[test]
        fn return_u64_in_stack_typed_array() {
            let trampoline = Win64::compile(&symbol(vec![U64, U64, U64, U64, U64], U64));
