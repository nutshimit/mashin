    ...
    pub fn try_from(mut item: syn::ItemMod, args: ProviderMetadataArgs) -> syn::Result<Self> {
        ...
        for (index, item) in items.iter_mut().enumerate() {
            ...
            match provider_attr {
                ...
                Some(ProviderAttr::Resource(name, config, span)) => resources.push(
                    resource::ResourceDef::try_from(name, config, span, index, item)?,
                ),
...
impl syn::parse::Parse for ProviderAttr {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if lookahead.peek(keyword::provider) {
            return Ok(ProviderAttr::Provider(
                content.parse::<keyword::provider>()?.span(),
            ));
        } else if lookahead.peek(keyword::config) {
            return Ok(ProviderAttr::Config(
                content.parse::<keyword::config>()?.span(),
            ));
        } else if lookahead.peek(keyword::builder) {
            return Ok(ProviderAttr::Builder(
                content.parse::<keyword::builder>()?.span(),
            ));
        } else if lookahead.peek(keyword::state) {
            return Ok(ProviderAttr::State(
                content.parse::<keyword::state>()?.span(),
            ));
        } else if lookahead.peek(keyword::calls) {
            return Ok(ProviderAttr::ResourceImpl(
                content.parse::<keyword::calls>()?.span(),
            ));
